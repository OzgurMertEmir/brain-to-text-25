...
sys.path.insert(0, str(REPO_ROOT))         # so 'models' is a top-level package
...
parser.add_argument("--lm-dir", default="language_model/pretrained_language_models/openwebtext_1gram_lm_sil")
...
# load model weights
checkpoint = torch.load(os.path.join(model_path, 'checkpoint/best_checkpoint'), weights_only=False, map_location=f'cuda:{gpu_number}')
...
for k, v in state.items():
    ...
    if new_k in ["rnn.weight_ih_l1", "rnn.weight_hh_l1", "rnn.bias_ih_l1", "rnn.bias_hh_l1", "rnn.weight_ih_l2", "rnn.weight_hh_l2", "rnn.bias_ih_l2", "rnn.bias_hh_l2", "rnn.weight_ih_l3", "rnn.weight_hh_l3", "rnn.bias_ih_l3", "rnn.bias_hh_l3", "rnn.weight_ih_l4", "rnn.weight_hh_l4", "rnn.bias_ih_l4", "rnn.bias_hh_l4"]:
        continue
    new_state[new_k] = v
...
dec = NgramDecoderWrapper(args.lm_dir,
...
with tqdm(total=total_test_trials, desc='Running remote language model', unit='trial') as pbar:
    for session in test_data.keys():
        for trial in range(len(test_data[session]['logits'])):
            ...
            best_candidate_sentence = dec.decode(logits_np, blank_penalty=7.0, return_nbest=False, do_rescore=False)
