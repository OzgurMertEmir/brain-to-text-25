...
def collapse_argmax_and_confidences(argmax_seq: np.ndarray, maxprob_seq: np.ndarray, blank_id: int = 0,
                                    agg_method: str = 'mean'):
    """
...
def compute_confidence_metrics_for_sample(logits_tensor: torch.Tensor, adjusted_len: int, true_phonemes: np.ndarray,
                                          blank_id: int = 0, agg_method: str = 'mean'):
    """
    ...
    pred_ids, pred_conf = collapse_argmax_and_confidences(argmax_np, maxprob_np, blank_id=blank_id,
    ...
    return {
        ...
        'top1_aligned': top1,
        'n_pred': len(pred_ids),
        'n_aligned': len(correct_flags),
        'n_matched': sum(correct_flags)
    }
...
def aggregate_batch_confidence_metrics(sample_metrics_list, pred_class_count):
    ...
    # compute stats
    per_class_mean_conf = {cls: (float(np.mean(vals)) if len(vals) > 0 else 0.0) for cls, vals in
                           per_class_confs.items()}
