...
def get_log_probs_from_logits(logits: np.ndarray) -> np.ndarray:
...
def compute_ctc_collapsed_confidence(log_probs: np.ndarray | torch.Tensor, path: List[int],
        blank_id: int = 0,agg: str = "mean") -> Tuple[List[int], List[float]]:
    
    # Convert torch → numpy for easy indexing
    ...
    for t, tok in enumerate(path):
        if tok == blank_id:
            continue  # ignore blanks completely
        
        frame_prob = np.exp(log_probs[t, tok])  # convert log → prob
        
        if tok != current_token:
            # flush previous aggregated token
            if current_token is not None:
                if agg == "mean":
                    collapsed_conf.append(float(np.mean(current_conf)))
                else:
                    collapsed_conf.append(float(np.max(current_conf)))
            
            current_token = tok
            collapsed_tokens.append(tok)
            current_conf = [frame_prob]
        else:
            current_conf.append(frame_prob)
    ...
    collapsed_conf = [np.round(x, 3) for x in collapsed_conf]
    return collapsed_tokens, collapsed_conf
